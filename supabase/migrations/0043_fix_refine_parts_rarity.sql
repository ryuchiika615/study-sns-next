-- Fix: empty part strings were matching all items via LIKE '%%', causing LR rarity
-- Change "is not null" to "!= ''" so empty parts don't participate in rarity calc
...
begin
  ...
  loop
    if p_word != '' and (v_item->>'name') like '%' || p_word || '%' then
    ...
    if p_noun != '' and (v_item->>'name') like '%' || p_noun || '%' then
      v_part_rarity := case v_item->>'rarity'
        when 'N' then 1 when 'R' then 2 when 'SR' then 3
        when 'SSR' then 4 when 'UR' then 5 when 'LR' then 6 else 1
      end;
    ...
    if p_name_part != '' and (v_item->>'name') like '%' || p_name_part || '%' then
      v_part_rarity := case v_item->>'rarity'
        when 'N' then 1 when 'R' then 2 when 'SR' then 3
        when 'SSR' then 4 when 'UR' then 5 when 'LR' then 6 else 1
      end;
