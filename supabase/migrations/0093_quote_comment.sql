-- Add quote_comment_id to posts
alter table posts add column if not exists quote_comment_id uuid references comments(id) on delete set null;

-- Update create_post RPC to accept quote_comment_id
...
begin
  ...
  insert into public.posts (user_id, content, subject, study_minutes, image_url, image_urls, quote_post_id, quote_comment_id, is_silent, audio_url, audio_name, created_at)
  values (v_user_id, p_content, p_subject, p_study_minutes, p_image_url, p_image_urls, p_quote_post_id, p_quote_comment_id, p_silent, p_audio_url, p_audio_name, v_created_at)
...
-- Update process_repost trigger to handle quote_comment_id
create or replace function process_repost()
returns trigger language plpgsql security definer set search_path = public as $$
declare
  v_original_author_id uuid;
begin
  if new.quote_post_id is null and new.quote_comment_id is null then
    return new;
  end if;

  -- Skip notifications for silent reposts
  ...
  if new.quote_comment_id is not null then
    select user_id into v_original_author_id from comments where id = new.quote_comment_id;
    if v_original_author_id = new.user_id then
      return new;
    end if;
