-- Fix buy_item and sell_items RPCs: uuid types, explicit gen_random_uuid, dedup checks
...
begin
  ...
  if v_existing is null then
    ...
    values (gen_random_uuid(), v_user_id, v_item.id);
  end if;
...
begin
  ...
  if p_max_rarity is not null then
    ...
    for v_rec in
      select ui.id as ui_id, ui.item_id, g.name, g.rarity, g.category
      from user_items ui
      join gacha_items g on g.id = ui.item_id
      where ui.user_id = v_user_id
    loop
      v_rarity_val := case v_rec.rarity
        when 'N' then 1 when 'R' then 2 when 'SR' then 3
        when 'SSR' then 4 when 'UR' then 5 when 'LR' then 6
        else 1
      end;
      ...
      if v_rarity_val <= v_max_val
         ...
         and v_rec.item_id != v_profile.current_title_id
         and v_rec.item_id != v_profile.current_avatar_id
      then
        v_ids_to_delete := array_append(v_ids_to_delete, v_rec.item_id);
        v_sell_val := case v_rec.rarity
