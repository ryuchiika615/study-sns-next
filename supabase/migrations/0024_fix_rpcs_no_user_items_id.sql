-- Fix all RPCs: user_items.id column does not exist in the database
-- Use ON CONFLICT DO NOTHING for dedup

-- Fix combine_items (no user_items.id reference)
...
begin
  ...
  if p_item_id_a = p_item_id_b then raise exception 'Cannot combine item with itself'; end if;
  ...
  if v_item_a is null or v_item_b is null then raise exception 'Item not found or not owned'; end if;
  ...
  if p_order = 'reverse' then v_left := v_item_b.name; v_right := v_item_a.name;
  else v_left := v_item_a.name; v_right := v_item_b.name; end if;
  ...
  if v_new_item is null then
    insert into gacha_items (name, rarity, category) values (v_full_title, v_new_rarity, 'title') returning * into v_new_item;
  end if;

  -- user_items has no id column; use ON CONFLICT to skip duplicates
...
-- Fix buy_item (no user_items.id reference)
