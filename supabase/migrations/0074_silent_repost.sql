-- Add is_silent column to posts for silent reposts
ALTER TABLE posts ADD COLUMN IF NOT EXISTS is_silent boolean NOT NULL DEFAULT false;

-- Update create_post RPC to accept p_silent parameter
...
begin
  ...
  insert into public.posts (user_id, content, subject, study_minutes, image_url, image_urls, quote_post_id, is_silent, created_at)
  values (v_user_id, p_content, p_subject, p_study_minutes, p_image_url, p_image_urls, p_quote_post_id, p_silent, v_created_at)
...
-- Update trigger: skip notification if is_silent
