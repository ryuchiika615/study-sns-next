...
export default function AchievementTitleForge({ onCreated, onMessage }: { onCreated: () => void; onMessage: (message: string) => void }) {
  ...
  return (
    <div className="rounded-xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-fuchsia-50 p-3">
      <div className="flex items-start justify-between gap-3">
        <div><h3 className="text-sm font-black text-violet-950">✦ 実績で五十音を解放</h3><p className="mt-0.5 text-[11px] leading-relaxed text-violet-800">あ〜んの文字を実績で集めよう。解放順・難易度はランダム。集めた文字を2〜8個並べて、自分だけの称号を作れます。</p></div>
      ...
      <div className="mt-3 rounded-lg bg-slate-950 px-3 py-2 text-center text-lg font-black tracking-[0.25em] text-white">{selectedCharacters || "称号を組み立てよう"}</div>
      ...
      {loading ? <p className="py-4 text-center text-xs text-gray-400">文字を読み込み中…</p> : (
        <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-5">
          {definitions.map((item) => {
            ...
            return <button key={item.id} type="button" onClick={() => toggle(item.id)} disabled={!unlocked}
