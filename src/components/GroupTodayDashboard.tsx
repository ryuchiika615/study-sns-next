...
export default function GroupTodayDashboard({ userId, groupId }: { userId: string; groupId: string }) {
  ...
  return (
    ...
    {showMinimizeConfirm && <div className="fixed inset-0 z-[90] flex items-end bg-slate-950/70 p-4 sm:items-center"><section className="mx-auto w-full max-w-md rounded-2xl bg-white p-5 text-slate-900 shadow-2xl"><p className="text-lg">✨</p><h2 className="mt-1 text-base font-black">今日のスタートを最小化しますか？</h2><p className="mt-2 text-sm leading-6 text-slate-600">このグループでは小さい表示になります。投稿やタイマーには影響しません。いつでも「表示設定」または「戻す」から元に戻せます。</p><div className="mt-4 flex gap-2"><button onClick={() => setShowMinimizeConfirm(false)} className="flex-1 rounded-xl border border-slate-200 py-3 text-sm font-bold text-slate-600">キャンセル</button><button onClick={minimize} className="flex-1 rounded-xl bg-slate-900 py-3 text-sm font-bold text-white">最小化する</button></div></section></div>}
    </>
  );
}

function DisplayGuide({ onClose, onRestore }: { onClose: () => void; onRestore: () => void }) {
  return <div className="fixed inset-0 z-[90] flex items-end bg-slate-950/70 p-4 sm:items-center"><section className="mx-auto w-full max-w-md rounded-2xl bg-white p-5 text-slate-900 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-base font-black">⚙️ 今日のスタートの表示設定</h2><button onClick={onClose} className="text-sm font-bold text-slate-500">閉じる</button></div><p className="mt-2 text-sm leading-6 text-slate-600">最小化しても、今日の案内は消えていません。下の小さいバーからいつでも元に戻せます。</p><div className="mt-4 rounded-2xl bg-slate-100 p-3"><p className="text-center text-[10px] font-bold text-slate-500">表示イメージ</p><div className="mt-2 rounded-xl bg-gradient-to-br from-emerald-900 to-cyan-950 p-3 text-white shadow"><p className="text-xs font-black">✨ 今日のスタート</p><div className="mt-2 grid grid-cols-3 gap-1"><span className="h-6 rounded bg-white/15" /><span className="h-6 rounded bg-white/15" /><span className="h-6 rounded bg-white/15" /></div></div><p className="my-2 text-center text-xl text-slate-400">↓</p><div className="flex items-center justify-between rounded-xl bg-slate-900 px-3 py-2 text-white"><span className="text-xs font-bold">✨ 今日のスタート　最小表示中</span><span className="rounded bg-emerald-500 px-2 py-1 text-[10px] font-bold">戻す</span></div></div><button onClick={() => { onRestore(); onClose(); }} className="mt-4 w-full rounded-xl bg-emerald-500 py-3 text-sm font-bold text-white">通常表示に戻す</button></section></div>;
}
