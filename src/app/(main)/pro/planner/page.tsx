...
export default function ProPlannerPage() {
  ...
  useEffect(() => { load(); }, []);
  const save = async () => { if (!examName.trim() || !examDate) return; const { error } = await supabase.from("pro_study_plans").insert({ exam_name: examName.trim(), exam_date: examDate, weekly_minutes: Number(weeklyMinutes) }); if (error) setMessage(error.message); else { setExamName(""); setExamDate(""); setMessage("計画を作成しました"); load(); } };
