...
export default function GroupsPage() {
  ...
  return <div className="mx-auto max-w-2xl space-y-4 p-4">
    ...
    <div className="space-y-2">{groups.length ? groups.map((group) => <Link key={group.id} href={`/groups/${group.id}`} className="block rounded-xl border border-gray-200 bg-white p-4 no-underline hover:border-blue-200"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-gray-900">{group.visibility === "private" ? "🔒" : "🌍"} {group.name}</p><p className="mt-1 text-xs text-gray-500">{group.description || "仲間と学習を続けるグループ"}</p></div><i className="fas fa-chevron-right text-gray-300 text-xs mt-2" /></div><p className="mt-3 text-[11px] font-bold text-blue-600"><i className="fas fa-chart-line mr-1" />グループ投稿・週間ランキング</p></Link>) : <div className="rounded-xl border border-dashed border-gray-300 bg-white py-10 text-center"><p className="text-2xl">👥</p><p className="mt-2 text-sm font-bold text-gray-700">まだ参加しているグループがありません</p><p className="mt-1 text-xs text-gray-500">仲間を招待して、学習を一緒に続けよう。</p></div>}</div>
  </div>;
}
