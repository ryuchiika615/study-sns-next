  ...
  return (
    <div className="mx-4 mb-3 space-y-3">
      {(totalMinutes > 0 || totalWorkoutMinutes > 0) && (
        <div className={`relative p-4 rounded-xl text-white text-center shadow-sm border ${
          showWorkout
            ...
            <div className="inline-flex bg-white/20 rounded-full p-0.5">
              <button onClick={() => setShowWorkout(false)}
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold transition cursor-pointer border-none ${
                  !showWorkout ? "bg-white text-blue-700 shadow-sm" : "text-white/70 bg-transparent"
                }`}>
                勉強
              </button>
              <button onClick={() => setShowWorkout(true)}
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold transition cursor-pointer border-none ${
                  showWorkout ? "bg-white text-pink-600 shadow-sm" : "text-white/70 bg-transparent"
                }`}>
                筋トレ
              </button>
            </div>
          </div>
          <p className={`text-sm mt-3 ${showWorkout ? "text-pink-200" : "text-blue-200"}`}>
