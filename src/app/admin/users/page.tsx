...
export default function AdminUsersPage() {
  ...
  const handleAction = async () => {
    ...
    else if (action === "adjust_points") body.value = inputValue;
  ...
  const modalContent = () => {
    ...
    switch (action) {
      ...
      case "adjust_points":
        return { title: "ポイント増減", placeholder: "例: 100 または -50", input: true };
      case "set_points":
              ...
              {users.map((u: any) => (
                <tr key={u.id} className={`border-t border-gray-100 ${u.is_banned ? "bg-red-50" : ""}`}>
                  <td className="px-3 py-2">
                    ...
                    {u.is_banned && <div className="text-xs text-red-600 font-bold mt-1">🚫 BAN中: {u.ban_reason || ""}</div>}
                  </td>
                  ...
                  <td className="px-3 py-2 text-center">
                    <button onClick={() => openModal(u, "adjust_points")}
                      className="text-xs px-2 py-1 rounded bg-green-50 text-green-600 hover:bg-green-100 cursor-pointer whitespace-nowrap">
                      増減
                    </button>
                  </td>
                  <td className="px-3 py-2 text-center">
